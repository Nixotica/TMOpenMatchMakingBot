...
from helpers import get_next_rank_for_player, get_rank_for_player
...
class LeaderboardView(ui.View):
    ...
    async def see_my_position(
        ...
        # Also just update the embed to avoid any confusing instances where
        # player sees different rank in response from leaderboard
        ...
        (first_listed_player_pos, nearby_players) = (
            self.ddb_manager.get_nearby_players_by_elo(
        ...
        for player in nearby_players:
            ...
            if not player_profile:
                nearby_players_value += (
                    f"**{pos}.** ({player.elo}) Deregistered Account\n"
                )
            else:
        ...
        embed.add_field(
            name=nearby_players_name, value=nearby_players_value, inline=False
        )

        next_rank = get_next_rank_for_player(
            player_elo.elo, self.leaderboard_id, leaderboard_ranks_descending
        )

        if next_rank is not None:
            next_rank_name = "Next Rank:"
            next_rank_value = f"{next_rank.display_name} - {next_rank.min_elo} "
            f"({next_rank.min_elo - player_elo.elo} away)"
    ...
    async def update_embed(self) -> None:
        ...
        if prev_player_rank is None:
            ...
            prev_player_rank = LeaderboardRank(
                "default", self.leaderboard_id, "Unranked", 0
            )
        ...
        for player_elo in top_25_player_elos:
            ...
            if player_rank != prev_player_rank:
                embed.add_field(
                    name=prev_player_rank.display_name,
                    value=rank_group_msg,
                )
                rank_group_msg = ""
            ...
            if player_profile is None:
                logging.error(
                    f"No player profile found for TM account ID "
                    f"{player_elo.tm_account_id} while updating leaderboard..."
                )
                continue
